...
async def get_points(start_point: Tuple[int, int], direction: str, length: int) -> List[Tuple[int, int]]:
    points = [start_point]
    for _ in range(1, length+1):
        points.append(OPS[direction](points[-1]))
    return points[1:]

async def build_points(movements: str) -> List[Tuple[int, int]]:
    points = [(0, 0)]
    for move in movements.split(','):
        direction, step = move[0], int(move[1:])
        points.extend(await get_points(points[-1], direction, step))
    return points[1:]

async def get_minimal_disctance(path1, path2, crossings):
    sums = []
    for crossing in crossings:
        idx1 = path1.index(crossing) + 1
        idx2 = path2.index(crossing) + 1
        # print(idx1, idx2)
        sums.append(idx1 + idx2)
    return min(sums)
...
async def main():
    ...
    for movements in inputs:
        results.append(await build_points(movements))
    intersection = set(results[0]) & set(results[1])
    return await get_minimal_disctance(results[0], results[1], intersection)
